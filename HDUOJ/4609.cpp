...
inline void solve() {
  ...
  while (T--) {
    ...
    for (int i = 0; i < p; i++) sum[i] = (ll)(f[i].real() / p + FFT::EPS);

    for (int i = 0; i < n; i++) sum[2 * v[i]]--;

    for (int i = 0; i < p; i++) sum[i] >>= 1;

    for (int i = 1; i < p; i++) sum[i] += sum[i - 1];

    ll ans = 0, tot = sum.back();
