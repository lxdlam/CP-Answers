...
Matrix<T> mul(Matrix<T> a, Matrix<T> b, T mod = 1e9 + 7) {
  ...
  for (int i = 0; i < temp.row; i++) {
    for (int k = 0; k < temp.col; k++) {
      auto t = a[i][k] % mod;
      for (int j = 0; j < temp.col; j++) temp[i][j] = (temp[i][j] + t * b[k][j] % mod) % mod;
    }
  }
...
inline void solve() {
  ...
  while (T--) {
    int n, k;
    cin >> n >> k;
    Matrix<int> mat(n, n), ans;
    for (auto& i : mat.data)
      for (auto& j : i) cin >> j;
    ans = fp(mat, k, 9973);
    int t = 0;
    for (int i = 0; i < ans.row; i++) t = (t + ans.data[i][i]) % 9973;
    cout << t << '\n';
  }
}
