...
int dfs(ll pos, int center, ll size) {
  if (pos == (size + 1) / 2) return center;
  if (pos > (size + 1) / 2) return dfs(pos - (size + 1) / 2, 1, size / 2);
  return dfs(pos, 0, size / 2);
}

// Pre-Build Function
void build() {
  FOR(i, 1, SIZE)
  twos[i] = (twos[i - 1] + 1) * 2 - 1;
}
