...
ll query(int ql, int qr, int l, int r, int cur) {
  ...
  ll ret = 0;
  if (ql <= m) ret = max(ret, query(ql, qr, l, m, cur << 1));
  if (qr > m) ret = max(ret, query(ql, qr, m + 1, r, cur << 1 | 1));
  return ret;
}
...
inline void solve() {
  ...
  for (int i = 0; i < n; i++) {
    dp[i] = A[i].len;
    // debug(query(1, A[i].l - 1, 1, SIZE, 1));
    if (A[i].l > 1) dp[i] += query(1, A[i].l - 1, 1, SIZE, 1);
    update(A[i].r, dp[i], 1, SIZE, 1);
  }
  ...
  for (int i = 0; i < n; i++) smax(ans, dp[i]);
