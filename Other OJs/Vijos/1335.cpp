...
int main() {
  ...
  while (n--) {
    ...
    isVaild = isRow(0);
    if (isVaild) isVaild = isCol(0);
    if (isVaild) isVaild = isSection(0);
...
bool isRow(int col) {
  ...
  for (int i = 0; i < 9; i++) {
    times[cells[i * 9 + col]]--;
    if (times[cells[i * 9 + col]] < 0) return false;
  }
...
bool isCol(int row) {
  ...
  for (int i = 0; i < 9; i++) {
    times[cells[row * 9 + i]]--;
    if (times[cells[row * 9 + i]] < 0) return false;
  }
...
bool isSection(int sec) {
  ...
  for (int i = sec / 3 * 3; i <= sec / 3 * 3 + 2; i++) {
    for (int j = sec % 3 * 3; j <= sec % 3 * 3 + 2; j++) {
      times[cells[i * 9 + j]]--;
      if (times[cells[i * 9 + j]] < 0) return false;
    }
  }
