...
void pushdown(int pos, int lt, int rt)
{
    if (mul[pos] != 1)
    {
        ...
        if (add[L])
            add[L] = (add[L] * mul[pos]) % P;
        if (add[R])
            add[R] = (add[R] * mul[pos]) % P;
...
void update(int ul, int ur, int type, ll val, int l, int r, int cur)
{
    ...
    if (ul <= m)
        update(ul, ur, type, val, l, m, L);
    if (ur > m)
        update(ul, ur, type, val, m + 1, r, R);
...
ll query(int ql, int qr, int l, int r, int cur)
{
    ...
    ll ans = 0;
    if (ql <= m)
        ans = (ans + query(ql, qr, l, m, L)) % P;
    if (qr > m)
        ans = (ans + query(ql, qr, m + 1, r, R)) % P;
