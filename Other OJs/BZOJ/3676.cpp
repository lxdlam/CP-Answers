...
const int SIZE = 3e5 + 10;  // String size

struct PAM {
  int node[SIZE][CHAR_SIZE];
  int fail[SIZE];
  int cnt[SIZE];  // the number of the palindrome of node i
...
inline void solve() {
  ...
  for (int i = 2; i <= pam.p; i++) smax(ans, 1LL * pam.len[i] * pam.cnt[i]);
  cout << ans << '\n';
}
