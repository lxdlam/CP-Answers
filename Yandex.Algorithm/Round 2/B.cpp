...
void solve() {
  ...
  debug(af, al, bf);
  for (int i = 0; i <= af; i++) ans += a[i] * base + b[0];
  for (int i = 1; i <= bf; i++) ans += a[af] * base + b[i];
  for (int i = af + 1; i <= al; i++) ans += a[i] * base + b[bf];
  for (int i = bf + 1; i < m; i++) ans += a[al] * base + b[i];
  for (int i = al + 1; i < n; i++) ans += a[i] * base + b[m - 1];
  cout << ans << endl;
}
