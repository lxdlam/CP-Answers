...
void mul(int a[][MSIZE], int b[][MSIZE]) {
  ...
  for (int i = 0; i < MSIZE; i++) {
    for (int j = 0; j < MSIZE; j++) {
      ll s = 0;
      for (int k = i; k < j + 1; k++) s += a[i][k] * b[k][j];
      tmp[i][j] = s % MOD;
    }
  }
...
void fp(int a[][MSIZE], int expr) {
  ...
  for (int i = 0; i < MSIZE; i++) MT[i][i] = 1;
...
// Pre-Build Function
inline void build() { memset(dp, -1, sizeof dp); }
...
inline void solve() {
  ...
  for (int i = 0; i < S; i++) {
    if (i < N24) {
      ...
      t[i][i] = A[i][i] = 25;
      if (i < N24 + N25 - 1) t[i][i + 1] = A[i][i + 1] = 1;  // To the next
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (m < i + n25) continue;  // There is no such a string

    int w = get(0, len - 1, i);
    ans = (ans + w * A[N24 - i][N24 + N25 + n25 - 1]) % MOD;

    if (n % 2 && flag) ans = (ans + MOD * MOD - w * C[N24 - i][N24 + n25 - 1]) % MOD;
  }
